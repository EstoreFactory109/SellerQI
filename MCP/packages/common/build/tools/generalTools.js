...
import { formatQueryStatus, formatErrorMessage } from "../utils/formatters.js";
...
export function registerGeneralTools(server) {
    ...
    server.tool("list-queries", "List all Data Kiosk queries with optional filtering", {
    ...
    server.tool("create-query", "Create a new GraphQL query for retrieving business analytics data", {
    ...
    server.tool("check-query-status", "Check the status of a specific query", {
    ...
    server.tool("cancel-query", "Cancel a running query", {
    ...
    server.tool("get-document-details", "Get details of a document, including the download URL", {
    ...
    server.tool("download-document", "Download the content of a document using its URL and save it to the local filesystem", {
...
The complete document has been saved and can be accessed at the path shown above.`
    ...
    server.tool("get-api-help", "Get help information about the Amazon Data Kiosk API", {
        topic: z.enum([
            "overview",
            "authentication",
            "queries",
...
If issues persist, check the [Amazon Selling Partner API documentation](https://developer-docs.amazon.com/sp-api/) or contact Amazon Seller Support.`;
    ...
    server.tool("read-file", "Read content from a file at the specified path", {
