 ...
 */
export function registerEconomicsTools(server) {
    // Tool to explore the economics schema
    server.tool("explore-economics-schema", "Explore the Seller Economics GraphQL schema structure to help build queries", {
        entityType: z.enum([
            "overview",
            "economics",
            "economicsPreview",
...
- Component breakdowns for certain fees provide visibility into fee structures`;
    ...
    server.tool("build-economics-query", "Build a GraphQL query for seller economics data based on parameters", {
        ...
        try {
            ...
            const query = buildEconomicsQuery(params.startDate, params.endDate, params.dateGranularity, params.productIdGranularity, [marketplaceId], params.includeFeeComponents, feeTypes);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("build-economics-preview-query", "Build a GraphQL query to preview future seller economics based on fee changes", {
        ...
        try {
            // Get the marketplace ID
            const marketplaceId = MARKETPLACES[params.marketplace];
            const query = buildEconomicsPreviewQuery(params.startDate, params.endDate, [marketplaceId], params.feeTypes);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("get-economics-example", "Get example GraphQL queries for seller economics data", {
        queryType: z.enum([
