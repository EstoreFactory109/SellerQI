    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (!intendedPackage || intendedPackage === 'null' || intendedPackage === 'undefined') {
          ...
          localStorage.removeItem('intendedPackage');
          try {
            const response = await axiosInstance.post('/app/complete-agency-signup', {}, { withCredentials: true });
            if (response.data.statusCode === 200 && response.data.data?.redirectTo) {
              localStorage.setItem('isAuth', 'true');
              ...
              setErrorMessage(response.data?.message || 'Failed to activate agency account. Please try again.');
              setLoading(false);
            }
          } catch (agencyError) {
            console.error('Complete agency signup error:', agencyError);
