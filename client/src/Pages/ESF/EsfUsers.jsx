...
const ROLE_BADGE = {
  owner: { label: 'Owner', className: 'border-amber-400/30 bg-amber-500/10 text-amber-300', Icon: Crown },
  admin: { label: 'Admin', className: 'border-blue-400/30 bg-blue-500/10 text-blue-300', Icon: Shield },
  member: { label: 'Member', className: 'border-white/10 bg-white/[0.035] text-gray-400', Icon: null },
};
...
const EsfUsers = () => {
  ...
  // Only the owner and admins may manage the team; members see it read-only.
  const canManageTeam = signedInUser?.esfRole === 'owner' || signedInUser?.esfRole === 'admin';
      ...
      <div className="relative max-w-[1600px] w-full">
        ...
        {!loading && !error && (
          <>
                      ...
                      return (
                        <tr key={user._id} className="group transition-colors hover:bg-white/[0.035] bg-transparent">
                          ...
                          <td className="px-2 py-2.5 text-center">
                            {(() => {
                              const badge = ROLE_BADGE[user.esfRole] || ROLE_BADGE.member;
                              const Icon = badge.Icon;
                              return (
                                <span className={`inline-flex items-center justify-center gap-1 rounded-full border px-2 py-1 text-xs font-medium ${badge.className}`}>
                          ...
                          <td className="px-2 py-2.5">
                            <div className="flex items-center justify-center">
                              {(!canManageTeam || user.isOwner) ? (
                                <span
                                  className="text-xs"
                                  style={{ color: 'rgb(107 114 128)' }}
                                  title={user.isOwner ? 'The portal owner cannot be modified' : 'Only the owner and admins can manage team members'}
                                >
                                  —
                                </span>
                              ) : (
            ...
            {/* Add user modal */}
            {showAddUserModal && canManageTeam &&
              createPortal(
                <div
