...
import { AlertTriangle, ChevronDown, Box, Eye, Activity, Star, TrendingUp, TrendingDown, LineChart, ArrowRight, Download, ArrowUpRight, ArrowDownRight, Minus, ImageOff, ArrowUp, ArrowDown, CheckCircle } from 'lucide-react';
...
const RankingIssuesTable = ({ product }) => {
    ...
    const [fixContext, setFixContext] = useState({
        ...
        hasBulletpointsIssue: false,
        hasBackendIssue: false,
        backendErrorType: null
    });
    const [generateTarget, setGenerateTarget] = useState('title');
    ...
    const [hasScrollControls, setHasScrollControls] = useState(false);
    ...
    useEffect(() => {
        if (!isFixModalOpen || !modalContentRef.current) {
            setHasScrollControls(false);
            return;
        }
        const el = modalContentRef.current;
        const hasScroll = el.scrollHeight > el.clientHeight + 1;
        setHasScrollControls(hasScroll);
    }, [isFixModalOpen, fixForm, fixContext]);
    ...
    const extractRankingErrors = (product) => {
        ...
        const fixedAttributes = product.fixedAttributes || {};
        const errorRows = [];
        ...
        // Backend Keywords - charLim (too long or too short)
        const backendCharLim = rankingErrors.charLim;
        ...
        if (rankingErrors.dublicateWords?.status === 'Error') {
            errorRows.push({
                issueHeading: 'Backend Keywords | Duplicate Words',
                message: rankingErrors.dublicateWords.Message,
                solution: rankingErrors.dublicateWords.HowTOSolve,
                attributeKey: 'backend',
    ...
    // Only consider unfixed issues for the modal - exclude already-fixed attributes
    const unfixedErrors = errors.filter(e => !e.isFixed);
    const hasTitleIssue = unfixedErrors.some(e => e.attributeKey === 'title');
    const hasDescriptionIssue = unfixedErrors.some(e => e.attributeKey === 'description');
    const hasBulletpointsIssue = unfixedErrors.some(e => e.attributeKey === 'bulletpoints');
    const hasBackendIssue = unfixedErrors.some(e => e.attributeKey === 'backend');
    const firstBackendError = unfixedErrors.find(e => e.attributeKey === 'backend');
    const backendErrorTypeFromErrors = firstBackendError?.backendErrorType || null;
    const displayedErrors = errors.slice(0, page * itemsPerPage);
    const hasMore = errors.length > displayedErrors.length;
    
    // Check if all issues have been fixed
    const allIssuesFixed = errors.length > 0 && errors.every(e => e.isFixed);

    const handleOpenFixModalForFirstIssue = () => {
        if (errors.length === 0) return;
        const newContext = {
            hasTitleIssue,
            hasDescriptionIssue,
            hasBulletpointsIssue,
            hasBackendIssue,
            backendErrorType: backendErrorTypeFromErrors
        };
        setFixContext(newContext);

        // Set default generate target to the first available issue type
        if (newContext.hasTitleIssue) {
            setGenerateTarget('title');
        } else if (newContext.hasDescriptionIssue) {
            setGenerateTarget('description');
        } else if (newContext.hasBulletpointsIssue) {
            setGenerateTarget('bulletpoints');
        } else if (newContext.hasBackendIssue) {
            setGenerateTarget('backend');
        }
        setFixForm({
            title: '',
            description: '',
    ...
    const handleFixSubmit = (e) => {
        ...
        if (!sku) {
            setApplyError('SKU is required to update the listing. This product may not have a SKU.');
            return;
        }
        if (!country || !region) {
        ...
        const addUpdate = (dataToBeUpdated, valueToBeUpdated) => {
            if (valueToBeUpdated === undefined || valueToBeUpdated === null) return;
            if (typeof valueToBeUpdated === 'string' && !valueToBeUpdated.trim()) return;
            if (Array.isArray(valueToBeUpdated) && valueToBeUpdated.length === 0) return;
            updates.push({ dataToBeUpdated, valueToBeUpdated });
        };

        if (fixContext.hasTitleIssue) {
            const titleValue = fixForm.title?.trim() || '';
            addUpdate('title', titleValue);
        }

        if (fixContext.hasDescriptionIssue) {
            const descValue = fixForm.description?.trim() || '';
            addUpdate('description', descValue);
        }

        if (fixContext.hasBackendIssue) {
            const backendValue = fixForm.backendKeywords?.trim() || '';
            addUpdate('generic_keyword', backendValue);
        }

        if (fixContext.hasBulletpointsIssue) {
            const bulletsRaw = Array.isArray(fixForm.bulletpoints)
                ? fixForm.bulletpoints
                : [fixForm.bulletpoints || ''];
            const bullets = bulletsRaw
                .map((b) => String(b ?? '').trim())
                .filter(Boolean);
            addUpdate('bulletpoints', bullets);
        }

        if (updates.length === 0) {
            setApplyError('Please enter at least one change to apply.');
            return;
        }

        setApplyLoading(true);
        Promise.all(
            updates.map(({ dataToBeUpdated, valueToBeUpdated }) =>
    ...
    const handleGenerateTitle = () => {
        if (!fixContext.hasTitleIssue) return;
        setGenerateTitleError(null);
    ...
    const handleGenerateDescription = () => {
        if (!fixContext.hasDescriptionIssue) return;
        setGenerateTitleError(null);
        setGenerateTitleLoading(true);
        axiosInstance.post('/api/ai/ranking-content', {
            asin: product.asin,
    ...
    const handleGenerateBulletPoints = () => {
        if (!fixContext.hasBulletpointsIssue) return;
        setGenerateTitleError(null);
        setGenerateTitleLoading(true);
    ...
    const handleGenerateBackendKeywords = () => {
        if (!fixContext.hasBackendIssue) return;
        setGenerateTitleError(null);
        setGenerateTitleLoading(true);
        axiosInstance.post('/api/ai/ranking-content', {
            asin: product.asin,
    ...
    const handleGenerateForTarget = () => {
        if (generateTarget === 'title') {
            handleGenerateTitle();
        } else if (generateTarget === 'description') {
            handleGenerateDescription();
        } else if (generateTarget === 'bulletpoints') {
            handleGenerateBulletPoints();
        } else if (generateTarget === 'backend') {
            handleGenerateBackendKeywords();
        }
    };

    // Generate all fields with issues in parallel
    const handleGenerateAll = async () => {
        ...
        if (fixContext.hasTitleIssue) {
            requests.push(
                ...
                .then(res => ({ field: 'title', res }))
                .catch(err => ({ field: 'title', err }))
            );
        }
        ...
        if (fixContext.hasDescriptionIssue) {
            requests.push(
                ...
                .then(res => ({ field: 'description', res }))
                .catch(err => ({ field: 'description', err }))
            );
        }

        if (fixContext.hasBulletpointsIssue) {
            const bulletsForApi = Array.isArray(fixForm.bulletpoints)
                ? fixForm.bulletpoints
                : [fixForm.bulletpoints || ''];
            requests.push(
                axiosInstance.post('/api/ai/ranking-content', {
                ...
                .then(res => ({ field: 'bulletpoints', res }))
                .catch(err => ({ field: 'bulletpoints', err }))
            );
        }
        ...
        if (fixContext.hasBackendIssue) {
            requests.push(
                ...
                .then(res => ({ field: 'backend', res }))
                .catch(err => ({ field: 'backend', err }))
            );
        }
        ...
        setFixForm(prev => {
            const next = { ...prev };
            results.forEach(({ field, res, err }) => {
                if (err) {
                    const msg = err.response?.data?.message || err.message || `Failed to generate ${field}`;
                ...
                if (field === 'bulletpoints' && data.bulletpoints) {
                    const arr = Array.isArray(data.bulletpoints)
                        ? data.bulletpoints
                        : [String(data.bulletpoints)];
                    next.bulletpoints = arr.length ? arr : [''];
                }
                if (field === 'backend' && data.keywords !== undefined) {
                    next.backendKeywords = data.keywords;
                }
            });
            return next;
        });
        ...
        if (errors.length > 0) {
            setGenerateTitleError(errors.join(' | '));
        }
    ...
    return (
        <div>
            <div className="w-full rounded border border-[#30363d]">
                <div className="flex items-center justify-between px-2 py-2 border-b border-[#30363d] bg-[#0d1117]">
                    <p className="text-xs font-semibold text-gray-300">Ranking Issues</p>
                    {allIssuesFixed ? (
                        <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded text-[11px] font-medium bg-blue-500/20 text-blue-400 border border-blue-500/30">
                            <CheckCircle className="w-3 h-3" />
                            Applied
                        </span>
                    ) : (
                        <button
                            className="inline-flex items-center px-3 py-1.5 rounded text-[11px] font-medium bg-blue-500 text-white hover:bg-blue-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                            onClick={handleOpenFixModalForFirstIssue}
                    ...
                    <tbody className="divide-y divide-[#30363d]">
                        {displayedErrors.map((error, idx) => (
                            <tr key={idx} className="border-b border-[#30363d] min-h-[60px]">
                                <td className="px-2 py-2 text-xs font-medium text-gray-100 align-top break-words leading-relaxed">
                                    <div className="flex items-center gap-1.5">
                                        <span>{error.issueHeading}</span>
                                        {error.isFixed && (
                                            <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[10px] font-medium bg-blue-500/20 text-blue-400 border border-blue-500/30">
            ...
            {/* Fix It modal for Issues by Product ranking tab */}
            <AnimatePresence>
                {isFixModalOpen && (
                    <motion.div
                    ...
                    >
                        <motion.div
                            className="bg-[#0d1117] border border-[#30363d] rounded-lg w-full max-w-lg mx-2 p-4 shadow-xl relative"
                        ...
                        >
                            ...
                            >
                                ...
                                {fixContext.hasDescriptionIssue && (
                                    <div>
                                        <label className="block text-xs font-medium text-gray-300 mb-1">
                                            New Description
                                        </label>
                                ...
                                {fixContext.hasBulletpointsIssue && (
                                    <div>
                                        <label className="block text-xs font-medium text-gray-300 mb-1">
                                            New Bullet Points
                                        </label>
                                ...
                                <div className="flex justify-end gap-2 pt-2">
                                    <button
                                        ...
                                        onClick={handleCloseFixModal}
                                        disabled={applyLoading || generateTitleLoading}
                                    >
                                        Cancel
                                    </button>
                                    <button
                                    ...
                                    >
                                        {generateTitleLoading ? 'Generating All…' : 'Generate All'}
                                    </button>
                                    <button
                                        type="submit"
                            ...
                            {/* Scroll controls (only when content is scrollable) */}
                            {hasScrollControls && (
                                <div className="hidden sm:flex flex-col gap-2 absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none">
                                    <button
                                        type="button"
                                        className="w-8 h-8 rounded-full bg-[#161b22]/80 border border-[#30363d] flex items-center justify-center text-gray-300 hover:bg-[#1f2937] hover:text-white transition-colors pointer-events-auto"
                                        onClick={() => {
...
const IssuesByProduct = () => {
    ...
    const viewProductDetails = (asin) => {
        navigate(`/seller-central-checker-demo/${asin}`);
    };
                                                ...
                                                <div className="flex items-center space-x-2">
                                                    <div className="relative">
                                                        {product.MainImage ? (
                                                            ...
                                                            <div className="w-12 h-12 rounded bg-[#21262d] border border-[#30363d] flex items-center justify-center text-gray-500">
