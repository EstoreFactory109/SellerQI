...
import { MessageSquare, Send, Plus, Trash2, Bot, User, Search, X, Check, Loader2, Wand2 } from 'lucide-react';
...
const APPLIED_SUCCESS_MESSAGE = 'Fix has been applied. You will be able to see the changes in your listing when Amazon accepts it.';

/**
 * ContentSuggestionsPanel - Displays content suggestions (titles, bullet points, etc.) 
 * with selection and apply functionality (Fix It feature for QMate)
 */
function ContentSuggestionsPanel({ 
  messageId,
  suggestions, 
  type, 
  asin, 
  ...
  const isTitleSuggestions = type === 'title' && Array.isArray(suggestions);
  const isBulletSuggestions = type === 'bulletpoints' && Array.isArray(suggestions);
  const isDescriptionSuggestion = type === 'description' && typeof suggestions === 'string';
  const isKeywordSuggestion = type === 'generic_keyword' && typeof suggestions === 'string';

  const isApplied = alreadyApplied === true;
  ...
  const handleApply = async () => {
    ...
    try {
      let valueToApply;
      if (isTitleSuggestions) {
        valueToApply = suggestions[selectedIndex];
      } else if (isBulletSuggestions) {
        valueToApply = suggestions;
      } else {
        valueToApply = suggestions;
      }
      
      await onApply(asin, sku, type, valueToApply);
      onApplySuccess?.(messageId, asin, type);
    } catch (err) {
  ...
  const getTypeLabel = () => {
    switch (type) {
      ...
      case 'generic_keyword': return 'Backend Keywords';
    ...
    <div className="mt-4 bg-[#161b22] border border-[#30363d] rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-gray-200">{getTypeLabel()}</span>
        </div>
        {productTitle && (
          <span className="text-xs text-gray-500 truncate max-w-[200px]" title={productTitle}>
            {productTitle}
          </span>
        )}
      </div>

      {/* Title Suggestions - Radio selection */}
      {isTitleSuggestions && (
          ...
          {suggestions.map((title, index) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <span className="text-xs text-gray-500 ml-2">({title.length} chars)</span>
              </div>
            </label>
          ))}
        </div>
        ...
        <div className="space-y-2 mb-4">
          {suggestions.map((bullet, index) => (
            ...
            >
              ...
              <span className="text-xs text-gray-500 mt-1 block ml-4">({bullet.length} chars)</span>
            </div>
          ))}
        </div>
      )}
      ...
      {/* Description */}
      {isDescriptionSuggestion && (
        <div className="mb-4 p-3 bg-[#0d1117] border border-[#30363d] rounded-lg max-h-40 overflow-y-auto">
          <p className="text-sm text-gray-200 whitespace-pre-wrap">{suggestions}</p>
          <span className="text-xs text-gray-500 mt-2 block">({suggestions.length} chars)</span>
        </div>
      )}
      ...
      {isKeywordSuggestion && (
        <div className="mb-4 p-3 bg-[#0d1117] border border-[#30363d] rounded-lg">
          <p className="text-sm text-gray-200 font-mono">{suggestions}</p>
          <span className="text-xs text-gray-500 mt-2 block">
            ({new TextEncoder().encode(suggestions).length} bytes)
          </span>
        </div>
      )}

      {/* Success message when fix has been applied */}
      {isApplied && (
      ...
      {/* Apply Button - disabled once applied for this ASIN */}
      <div className="flex items-center gap-3">
        {sku ? (
          ...
          <div className="text-xs text-amber-400 bg-amber-400/10 px-3 py-2 rounded-lg">
            SKU not found. Please apply this change manually from the Issues page.
          </div>
        )}
        
        {asin && (
          <span className="text-xs text-gray-500">ASIN: {asin}</span>
        )}
      </div>
...
const QMate = () => {
  ...
  const [showTryFreeTrialPopup, setShowTryFreeTrialPopup] = useState(false);
  const [pendingAction, setPendingAction] = useState(null);
  const [actionLoading, setActionLoading] = useState(false);
  /** Tracks which suggestion panels have had fix applied (messageId-asin-type) so Apply stays disabled */
  const [appliedFixKeys, setAppliedFixKeys] = useState(() => new Set());
  const messagesEndRef = useRef(null);
  const inputRef = useRef(null);

  // Get dashboard date range from Redux to sync QMate data with dashboard filters
  ...
  // Handle content actions (Fix It functionality)
  const handleGenerateSuggestion = async (asin, attribute, currentValue) => {
    setActionLoading(true);
    try {
      const response = await axiosInstance.post('/api/qmate/generate-suggestion', {
        asin,
        attribute,
        currentValue
      });
      console.log('[QMate] Generated suggestion:', response.data);
      return response.data?.data?.suggestions;
    } catch (error) {
      console.error('[QMate] Failed to generate suggestion:', error);
      throw error;
    } finally {
  ...
  const handleApplyFix = async (asin, sku, attribute, value) => {
    ...
    try {
      ...
      console.log('[QMate] Applied fix:', response.data);
      return response.data?.data;
    } catch (error) {
      console.error('[QMate] Failed to apply fix:', error);
      throw error;
    } finally {
      setActionLoading(false);
    }
  ...
  const handleContentAction = async (action) => {
    if (action.action === 'generate_suggestion') {
      return handleGenerateSuggestion(action.asin, action.attribute, action.current_value);
    } else if (action.action === 'apply_fix') {
      return handleApplyFix(action.asin, action.sku, action.attribute, action.suggested_value);
    }
    return null;
  };

  /** Record that a fix was applied for this message + asin + type so the Apply button stays disabled */
  const markFixApplied = (messageId, asin, type) => {
    if (!messageId || !asin || !type) return;
    const key = `${messageId}-${asin}-${type}`;
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (assistantPayload.content_actions && assistantPayload.content_actions.length > 0) {
        for (const action of assistantPayload.content_actions) {
          if (action.action === 'generate_suggestion') {
            try {
              const suggestionResponse = await axiosInstance.post('/api/qmate/generate-suggestion', {
                asin: action.asin,
                attribute: action.attribute,
                currentValue: action.current_value
              });
              console.log('[QMate] Generated suggestion:', suggestionResponse.data);
            } catch (suggestionError) {
              console.error('[QMate] Failed to generate suggestion:', suggestionError);
            }
          } else if (action.action === 'apply_fix') {
            try {
              const applyResponse = await axiosInstance.post('/api/qmate/apply-fix', {
                asin: action.asin,
                sku: action.sku,
                attribute: action.attribute,
                value: action.suggested_value
              });
              console.log('[QMate] Applied fix:', applyResponse.data);
            } catch (applyError) {
              console.error('[QMate] Failed to apply fix:', applyError);
            }
          }
        }
      }
  ...
  return (
    <div className="flex h-full min-h-0 bg-[#0f0f0f] text-gray-100 overflow-hidden">
              ...
              <div className="space-y-6">
                ...
                >
                  ...
                  >
                    ...
                    {/* Content Suggestions Panel - Fix It functionality */}
                    {message.role === 'assistant' && message.suggestedTitle && Array.isArray(message.suggestedTitle) && message.suggestedTitle.length > 0 && (
                      <ContentSuggestionsPanel
                        messageId={message.id}
                        suggestions={message.suggestedTitle}
                        type="title"
                        asin={message.contentActions?.[0]?.asin}
                        sku={message.contentActions?.[0]?.sku}
                        productTitle={message.contentActions?.[0]?.product_title}
                        onTryFreeTrial={() => setShowTryFreeTrialPopup(true)}
                        onApply={handleApplyFix}
                        onApplySuccess={markFixApplied}
                        isApplying={actionLoading}
                        alreadyApplied={appliedFixKeys.has(`${message.id}-${message.contentActions?.[0]?.asin}-title`)}
                      />
                    )}

                    {message.role === 'assistant' && message.suggestedBulletPoints && Array.isArray(message.suggestedBulletPoints) && message.suggestedBulletPoints.length > 0 && (
                      <ContentSuggestionsPanel
                        messageId={message.id}
                        suggestions={message.suggestedBulletPoints}
                        type="bulletpoints"
                        asin={message.contentActions?.[0]?.asin}
                        sku={message.contentActions?.[0]?.sku}
                        productTitle={message.contentActions?.[0]?.product_title}
                        ...
                        onApplySuccess={markFixApplied}
                        isApplying={actionLoading}
                        alreadyApplied={appliedFixKeys.has(`${message.id}-${message.contentActions?.[0]?.asin}-bulletpoints`)}
                      />
                    )}

                    {message.role === 'assistant' && message.suggestedDescription && typeof message.suggestedDescription === 'string' && (
                      <ContentSuggestionsPanel
                        messageId={message.id}
                        suggestions={message.suggestedDescription}
                        type="description"
                        asin={message.contentActions?.[0]?.asin}
                        sku={message.contentActions?.[0]?.sku}
                        productTitle={message.contentActions?.[0]?.product_title}
                        ...
                        onApplySuccess={markFixApplied}
                        isApplying={actionLoading}
                        alreadyApplied={appliedFixKeys.has(`${message.id}-${message.contentActions?.[0]?.asin}-description`)}
                      />
                    )}

                    {message.role === 'assistant' && message.suggestedBackendKeywords && typeof message.suggestedBackendKeywords === 'string' && (
                      <ContentSuggestionsPanel
                        messageId={message.id}
                        suggestions={message.suggestedBackendKeywords}
            ...
            <div className="relative flex items-end gap-2">
              <div className="flex-1 relative">
                <textarea
                  ...
                  onClick={() => setShowTryFreeTrialPopup(true)}
                  onFocus={() => setShowTryFreeTrialPopup(true)}
                  onKeyDown={(e) => {
      ...
      {/* Try free trial popup for demo actions */}
      <DemoTryFreeTrialPopup
        open={showTryFreeTrialPopup}
        onClose={() => setShowTryFreeTrialPopup(false)}
      />
