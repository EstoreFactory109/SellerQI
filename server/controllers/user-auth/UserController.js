...
const loginUser = asyncHandler(async (req, res) => {
    ...
    if (!checkUserIfExists.password) {
        // Passwordless no longer implies Google: agency clients never get a password, and
        // ESF clients created before passwords were added have none either. Sending those
        // users to Google Sign-In would be a dead end, so split on how the account was made.
        ...
        logger.warn(`Login attempt on managed account with no password set: ${checkUserIfExists.email}`);
    ...
    // Placed after password verification on purpose: answering this pre-auth would
    // disclose that an address exists and is ESF staff, re-opening the enumeration
    // leak closed in f61149f. A staff member with the right password still lands here.
