...
const getReport = async (accessToken, marketplaceIds, userId, baseuri, country, region) => {
    ...
    try {
        ...
        while (!reportDocumentId && retries > 0) {
            logger.debug(`[GET_FBA_REIMBURSEMENTS_DATA] Checking report status... (Retries left: ${retries})`);
        ...
        return await _processReimbursementsDocument(accessToken, reportDocumentId, baseuri, userId, country, region);
...
async function _processReimbursementsDocument(accessToken, reportDocumentId, baseuri, userId, country, region) {
...
// P8: Non-blocking async adapter — reuses _processReimbursementsDocument (same code as inline).
const { checkSpApiStatusOnce } = require('./spApiReportAdapter.js');
getReport.spApiAsync = {
    serviceName: 'fbaReimbursementsData',
    buildSpecs: ({ userId, country, region, accessToken, baseuri, marketplaceIds }) => ([{
        service: 'fbaReimbursementsData',
        paramsKey: 'default',
        params: {},
        marketplaceId: '',
        ...
        finalize: async (handle) => {
            await _processReimbursementsDocument(accessToken, handle.reportDocumentId, baseuri, userId, country, region);
            return { empty: false };
        },
    }]),
