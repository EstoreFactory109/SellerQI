...
const getReport = async (accessToken, marketplaceIds, userId, baseuri, country, region) => {
    ...
    try {
        ...
        return await _processLedgerSummaryDocument(accessToken, reportDocumentId, baseuri, userId, country, region);
...
async function _processLedgerSummaryDocument(accessToken, reportDocumentId, baseuri, userId, country, region) {
...
// P8: Non-blocking async adapter — reuses _processLedgerSummaryDocument (same code as inline).
const { checkSpApiStatusOnce } = require('./spApiReportAdapter.js');
getReport.spApiAsync = {
    serviceName: 'ledgerSummaryViewData',
    buildSpecs: ({ userId, country, region, accessToken, baseuri, marketplaceIds }) => ([{
        service: 'ledgerSummaryViewData',
        paramsKey: 'default',
        params: {},
        marketplaceId: '',
        ...
        finalize: async (handle) => {
            await _processLedgerSummaryDocument(accessToken, handle.reportDocumentId, baseuri, userId, country, region);
            return { empty: false };
        },
    }]),
