    ...
    try {
        ...
        if (firstBytes[0] === 0x1f && firstBytes[1] === 0x8b) {
            try {
                ...
                logger.error("[GET_LEDGER_DETAIL_VIEW_DATA] Failed to decompress gzipped data:", decompressError.message);
        ...
        if (!tsv || tsv.trim().length === 0) {
            logger.warn('[GET_LEDGER_DETAIL_VIEW_DATA] TSV buffer is empty');
            return [];
        }
        ...
        if (tsv.length > 0) {
            ...
            logger.info(`[GET_LEDGER_DETAIL_VIEW_DATA] TSV Preview (first 500 chars): ${preview}`);
        }
        ...
        logger.info('[GET_LEDGER_DETAIL_VIEW_DATA] TSV parsed successfully', { 
