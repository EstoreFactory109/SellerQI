...
class ScheduledIntegration {
    ...
    static async getSellerDataAndTokens(userId, Region, Country) {
        try {
            // Project only the account fields read below (tokens/ProfileId/sellerId) and
            // use .lean() — avoids hydrating the full Seller doc incl. the large embedded
            // products[] array on every phase. This function never mutates/saves the doc.
            const getSellerData = await Seller.findOne(
                { User: userId },
                {
                    ...
                    'sellerAccount.ProfileId': 1,
                    'sellerAccount.selling_partner_id': 1
                }
            ).lean();
    ...
    static async generateCredentials(regionConfig, loggingHelper) {
        ...
        try {
            // Reuse still-valid STS credentials across phases of the same run (in-process
            // only — these hold AWS secret keys and must not go to shared Redis). TTL is
            // <STS expiry, so a cached cred is always valid; a miss regenerates as before.
            let credentials = authCache.getCredentials(regionConfig);
            ...
            if (!credentials) {
                ...
                authCache.setCredentials(regionConfig, credentials);
            }
    ...
    static async generateTokens(userId, RefreshToken, AdsRefreshToken, loggingHelper) {
        ...
        try {
            ...
            // Cross-phase token cache: reuse a still-valid access token generated by an
            // earlier phase of this same daily run instead of re-hitting LWA every phase.
            // A cache hit is fed in as an already-resolved promise so the Promise.allSettled
            // result handling below is unchanged. Cached tokens are <50min old (TTL) and
            // thus always within Amazon's 60min expiry; a miss/Redis error falls through to
            // a fresh generation (identical to the previous behaviour).
            const spCached = RefreshToken ? await authCache.getToken('sp', RefreshToken) : null;
            const adsCached = AdsRefreshToken ? await authCache.getToken('ads', AdsRefreshToken) : null;

            if (RefreshToken) {
                tokenPromises.push(spCached ? Promise.resolve(spCached) : generateAccessToken(userId, RefreshToken, spapiErr));
                tokenTypes.push('SP-API');
            }

            if (AdsRefreshToken) {
                const { generateAdsAccessToken } = require('../AmazonAds/GenerateToken.js');
                tokenPromises.push(adsCached ? Promise.resolve(adsCached) : generateAdsAccessToken(AdsRefreshToken));
                tokenTypes.push('Ads');
            }
    ...
    static async _fetchProductArraysFromDB(userId, country, region) {
        ...
        // Project only the product sub-fields used to build the arrays (asin/sku/status),
        // plus country/region for account matching — avoids loading the heavy per-product
        // fields (issues[], itemName, price, etc.) that this function never reads.
        const seller = await Seller.findOne(
            { User: userId },
            {
                'sellerAccount.country': 1,
                'sellerAccount.region': 1,
                'sellerAccount.products.asin': 1,
                'sellerAccount.products.sku': 1,
                'sellerAccount.products.status': 1
            }
        ).lean();
